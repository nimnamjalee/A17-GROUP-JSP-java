...
<head>
    ...
    <meta charset="UTF-8">
    <title>Retrieve Order Details</title>
    <style>
        h1 {
            color: #1A3B15;
            text-align: center;
...
<%
    ...
    if (request.getMethod().equalsIgnoreCase("post")) {
        ...
        try {
            ...
            // Execute a SQL statement to retrieve orders for the specified username
            String sql = "SELECT * FROM orderby WHERE username=?";
            try (PreparedStatement pstmt = conn.prepareStatement(sql)) {
                ...
                if (rs.next()) {
                    // Display all orders for the specified username
                    out.println("<h2>Orders for Username: " + username + "</h2>");
                    do {
                        out.println("<p>Order ID: " + rs.getInt("id") + "</p>");
                        out.println("<p>Email: " + rs.getString("email") + "</p>");
                        out.println("<p>Location: " + rs.getString("location") + "</p>");
                        out.println("<p>Payment Method: " + rs.getString("payment_method") + "</p>");
                        out.println("<p>Timestamp: " + rs.getTimestamp("timestamp") + "</p>");
                        out.println("<hr>");
                    } while (rs.next());
                } else {
                    out.println("<p>No orders found for the specified username.</p>");
                }
            }
...
<br><br><!-- comment -->
 <a href="cancel.jsp" class="next"><button>cancel order</button></a>
 <br><br>
  <h5>Useful Links</h5>
